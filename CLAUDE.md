...
11. ✅ Вкладка «Загрузка БА» объединена с «Детализацией» (2026-06-30) — была статичным JSON-снимком
    ...
    - ✅ Уточнено по более точному источнику (реальный CSV-реестр с CAB-тикетами, 2026-07-01):
      9 задач БА оказались дублями уже существующих реальных проектов (тот же CAB, назначенный
      другой командой) — смёржены в реальные проекты вместо отдельных карточек (например,
      Троян/«Планирование прогнозирование» → реальный «Калькулятор Импорта», CAB-8545); один
      genuine пропущенный кусок работы (Троян/«МПК», CAB-8483) добавлен как новое назначение;
      добавлены CAB-ссылки ещё на 3 карточки, у которых их раньше не было. Даты/% всех 30+1
      назначений БА пересчитаны по более точным (duration-weighted) данным реестра вместо
      грубого равновзвешенного среднего из первоначальной миграции
